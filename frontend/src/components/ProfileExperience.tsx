...
export function ProfileExperience({
  ...
  const heroStats = useMemo(
    ...
    [data.experience.length, data.projects.length, data.skills.length],
  )
  ...
  const pageClassName = preview ? 'page profile-page cyber-page profile-preview-page' : 'page profile-page cyber-page'
  const topbarClassName = preview ? 'cyber-topbar profile-preview-topbar' : 'cyber-topbar'
  const badgeClassName = preview ? 'view-counter-badge profile-preview-badge' : 'view-counter-badge'
  ...
  return (
    <div className={pageClassName}>
      ...
      <header className={topbarClassName}>
        {preview ? (
          <span className="brand-mark">{firstName}.</span>
        ) : (
        ...
        <nav className="cyber-nav">
          {preview
            ? navItems.map((item) => (
                <span className="profile-preview-nav-pill" key={item.href}>
                  {item.label}
                </span>
              ))
            : navItems.map((item) => (
      ...
      <section className="hero-grid section-anchor" id={preview ? undefined : 'home'}>
      ...
      <section className="about-grid section-anchor" id={preview ? undefined : 'about'}>
      ...
      <section className="panel service-panel section-anchor" id={preview ? undefined : 'service'}>
        <SectionTitle
          eyebrow="Service"
      ...
      <section className="project-section section-anchor" id={preview ? undefined : 'project'}>
        <div className="section-title">
          <span>Project</span>
          <h2>Selected builds from the current stack</h2>
          <p>
            Projects stay data-driven, but the presentation is now shaped like a cinematic landing page
            instead of a neutral dashboard.
          </p>
        </div>
      ...
      <section className="skill-layout section-anchor" id={preview ? undefined : 'skill'}>
      ...
      <footer className="panel contact-panel section-anchor" id={preview ? undefined : 'contact'}>
        <div className="contact-copy">
          <p className="card-kicker">Contact</p>
          <h2>Ready to ship a darker, sharper portfolio presence.</h2>
          <p>
            Frontend deploys on Vercel, profile data can come from Railway, and the page still renders
            safely if the API is not available.
          </p>
        </div>
