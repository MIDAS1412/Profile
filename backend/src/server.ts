...
function buildProfileViewsResponse(stats: ProfileViewStats): ProfileViewsResponse {
  return {
    ok: true,
    ...stats,
  }
...
app.get(['/profile/views', '/api/profile/views'], async (_request, response) => {
  const stats = await loadProfileViews()
